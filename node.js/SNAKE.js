...
const nextApple = (state) =>{
    const check = (state) =>{
        state.snake.forEach(coord =>{
            if(coord[0] == state.appleX && coord == state.appleY) return false;
        })
        return true;
    }
    state.appleX = rnd(0)(14)
    state.appleY = rnd(0)(20)
...
const addApple = (state) => (board) =>{
    return board.map((r,i) =>{
        return i != state.appleX ? r : r.map((v,j)=>{
            return j == state.appleY ? chalk.redBright("○") : v
        })
    })
}
let gameState = {
...
const eatApple = (state) =>{
    if(state.appleX == state.snake[0][0] && state.appleY == state.snake[0][1]){ 
...
setInterval(()=>{
    let header = newLine();
    header += chalk.greenBright(row(' ')(12) + 'SNAKE '+ row(' ')(12))
    console.log(header)
    let board = col(row('∘')(20))(15).split('\n').map((x) => x.split(''))
